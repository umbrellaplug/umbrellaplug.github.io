...
import inspect
from umbrellascrapers.modules.control import transPath, setting as getSetting, lang, joinPath, existsPath
...
DEBUGPREFIX = '[COLOR red][ UMBRELLASCRAPERS %s ][/COLOR]'
...
def log(msg, caller=None, level=LOGINFO):
	...
	try:
		...
		if debug_location == '1':
			log_file = joinPath(LOGPATH, 'umbrellascrapers.log')
		...
		traceback.print_exc()
		import xbmc
		xbmc.log('[ script.module.umbrellaonscrapers ] log_utils.log() Logging Failure: %s' % (e), LOGERROR)
...
def error(message=None, exception=True):
	try:
		...
		if exception:
			...
			addon = 'script.module.umbrellascrapers'
...
def clear_logFile():
	cleared = False
	try:
		from umbrellascrapers.modules.control import yesnoDialog
...
def view_LogFile(name):
	try:
		...
		from umbrellascrapers.modules.control import addonPath
		log_file = joinPath(LOGPATH, '%s.log' % name.lower())
		if not existsPath(log_file):
			from umbrellascrapers.modules.control import notification
...
def upload_LogFile():
	...
	try:
		import requests
		from umbrellascrapers.modules.control import addonVersion, selectDialog
		f = open(log_file, 'r', encoding='utf-8', errors='ignore')
		text = f.read()
		f.close()
		UserAgent = 'UmbrellaScrpaers %s' % addonVersion()
		...
		if 'key' in response.json():
			...
			log('umbrellascrapers log file uploaded to: %s' % result)
			...
			if 'Copy url To Clipboard' in list[select][0]:
				from umbrellascrapers.modules.source_utils import copy2clip
				copy2clip(list[select - 1][1])
		elif 'message' in response.json():
			notification(message='umbrellascrapers Log upload failed: %s' % str(response.json()['message']))
			log('umbrellascrapers Log upload failed: %s' % str(response.json()['message']), level=LOGERROR)
		else:
			notification(message='umbrellascrapers Log upload failed')
			log('umbrellascrapers Log upload failed: %s' % response.text, level=LOGERROR)
	except:
		error('umbrellascrapers log upload failed')
