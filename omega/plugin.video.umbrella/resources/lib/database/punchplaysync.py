# -*- coding: utf-8 -*-
"""PunchPlay local watched-state, list and bookmark cache."""
...
from resources.lib.modules.control import existsPath, dataPath, makeFile, punchplaySyncFile
from resources.lib.modules import cleandate

def get_connection(setRowFactory=False):
	if not existsPath(dataPath): makeFile(dataPath)
	dbcon = db.connect(punchplaySyncFile, timeout=60)
...
def delete_punchplay_tables(tables):
	# Delete and vacuum the specified PunchPlay tables, and reset their service timestamps.
	try:
...
#status-bucket shape, but keyed by list_id instead of one table per bucket since PunchPlay's lists are
#arbitrary and user-named rather than a fixed Watchlist/Watching/etc set. No activity/last-modified
#signal exists for this endpoint (unlike Trakt), so this is polled on punchplay_syncInterval and always
...
def fetch_user_lists(media_type):
	...
	try:
		dbcon = get_connection()
		dbcur = get_connection_cursor(dbcon)
		ck_table = dbcur.execute('''SELECT * FROM sqlite_master WHERE type='table' AND name='punchplay_lists';''').fetchone()
		if not ck_table: return result
		match = dbcur.execute('''SELECT list_id, list_name, COUNT(*) FROM punchplay_lists WHERE media_type=? GROUP BY list_id, list_name''', (media_type,)).fetchall()
...
def fetch_list_items(list_id, media_type):
	...
	try:
		...
		ck_table = dbcur.execute('''SELECT * FROM sqlite_master WHERE type='table' AND name='punchplay_lists';''').fetchone()
		if not ck_table: return result
		match = dbcur.execute('''SELECT title, year, tmdb, item_id, listed_at FROM punchplay_lists WHERE list_id=? AND media_type=?''', (str(list_id), media_type)).fetchall()
...
def insert_user_lists(rows):
	...
	try:
		dbcon = get_connection()
		dbcur = get_connection_cursor(dbcon)
		dbcur.execute('''CREATE TABLE IF NOT EXISTS punchplay_lists (list_id TEXT, list_name TEXT, item_id TEXT, tmdb TEXT, title TEXT, year TEXT, media_type TEXT, listed_at TEXT, UNIQUE(list_id, tmdb, media_type));''')
		dbcur.execute('''DELETE FROM punchplay_lists''')
		dbcur.connection.commit()
		for r in rows:
			try:
				dbcur.execute('''INSERT OR REPLACE INTO punchplay_lists Values (?, ?, ?, ?, ?, ?, ?, ?)''',
...
#PunchPlay is TMDB-native so imdb/tvdb columns are kept for cross-lookup convenience but tmdb is authoritative.

def _ensure_watched_tables(dbcur):
	dbcur.execute('''CREATE TABLE IF NOT EXISTS punchplay_watched_movies (imdb TEXT, tmdb TEXT, title TEXT, year TEXT, last_watched_at TEXT, UNIQUE(tmdb));''')
	dbcur.execute('''CREATE TABLE IF NOT EXISTS punchplay_watched_episodes (show_imdb TEXT, show_tmdb TEXT, show_tvdb TEXT, season INTEGER, episode INTEGER, last_watched_at TEXT, UNIQUE(show_tmdb, season, episode));''')
	dbcur.execute('''CREATE TABLE IF NOT EXISTS service (setting TEXT, value TEXT, UNIQUE(setting));''')
...
def upsert_watched_movie(imdb='', tmdb='', title='', year='', last_watched_at=''):
	try:
		...
		dbcur.execute('''INSERT OR REPLACE INTO punchplay_watched_movies Values (?, ?, ?, ?, ?)''', (imdb, str(tmdb), title, str(year), last_watched_at))
		dbcur.connection.commit()
	except:
		from resources.lib.modules import log_utils
		log_utils.error()
...
def upsert_watched_episode(show_imdb='', show_tmdb='', show_tvdb='', season=0, episode=0, last_watched_at=''):
	try:
		...
		dbcur = get_connection_cursor(dbcon)
		_ensure_watched_tables(dbcur)
		dbcur.execute('''INSERT OR REPLACE INTO punchplay_watched_episodes Values (?, ?, ?, ?, ?, ?)''', (show_imdb, str(show_tmdb), str(show_tvdb), int(season), int(episode), last_watched_at))
		dbcur.connection.commit()
	except:
		from resources.lib.modules import log_utils
		log_utils.error()
...
def bulk_upsert_watched_movies(rows):
	...
	try:
		...
		dbcur = get_connection_cursor(dbcon)
		_ensure_watched_tables(dbcur)
		dbcur.executemany('''INSERT OR REPLACE INTO punchplay_watched_movies Values (?, ?, ?, ?, ?)''', rows)
		dbcur.connection.commit()
	except:
		from resources.lib.modules import log_utils
		log_utils.error()
...
def bulk_upsert_watched_episodes(rows):
	...
	try:
		...
		dbcur = get_connection_cursor(dbcon)
		_ensure_watched_tables(dbcur)
		dbcur.executemany('''INSERT OR REPLACE INTO punchplay_watched_episodes Values (?, ?, ?, ?, ?, ?)''', rows)
		dbcur.connection.commit()
	except:
		from resources.lib.modules import log_utils
		log_utils.error()
...
def delete_watched_movie(tmdb):
	try:
		...
		dbcur = get_connection_cursor(dbcon)
		_ensure_watched_tables(dbcur)
		dbcur.execute('''DELETE FROM punchplay_watched_movies WHERE tmdb=?''', (str(tmdb),))
		dbcur.connection.commit()
	except:
		from resources.lib.modules import log_utils
		log_utils.error()
...
def delete_watched_episode(show_tmdb, season, episode):
	try:
		...
		dbcur = get_connection_cursor(dbcon)
		_ensure_watched_tables(dbcur)
		dbcur.execute('''DELETE FROM punchplay_watched_episodes WHERE show_tmdb=? AND season=? AND episode=?''', (str(show_tmdb), int(season), int(episode)))
		dbcur.connection.commit()
	except:
		from resources.lib.modules import log_utils
		log_utils.error()
...
def get_watched_movies():
	...
	try:
		...
		rows = dbcur.execute("SELECT imdb FROM punchplay_watched_movies WHERE imdb != ''").fetchall()
...
def get_watched_movies_full():
	...
	try:
		...
		dbcur = get_connection_cursor(dbcon)
		_ensure_watched_tables(dbcur)
		rows = dbcur.execute("SELECT imdb, tmdb, title, year, last_watched_at FROM punchplay_watched_movies WHERE NOT imdb='' ORDER BY last_watched_at DESC").fetchall()
...
def get_watched_episodes():
	...
	try:
		...
		dbcur = get_connection_cursor(dbcon)
		_ensure_watched_tables(dbcur)
		rows = dbcur.execute('''SELECT show_imdb, show_tmdb, show_tvdb, season, episode FROM punchplay_watched_episodes''').fetchall()
		result = [(r[0], r[1], r[2], r[3], r[4]) for r in rows]
	except:
		from resources.lib.modules import log_utils
		log_utils.error()
...
def get_watched_shows():
	...
	try:
		...
		rows = dbcur.execute('''SELECT show_imdb, show_tmdb, show_tvdb, MAX(last_watched_at) AS last_watched_at
			FROM punchplay_watched_episodes GROUP BY show_tmdb ORDER BY last_watched_at DESC''').fetchall()
...
def clear_cache():
	...
	# delete_punchplay_tables() also resets last_history_at to epoch as a side effect, which
...
# Resume cache mirrors server playback records and successful local playback events.
	...
	try:
		...
		if ret_all:
			...
			if episode is None or episode == '':
				try:
					match = dbcur.execute('''SELECT * FROM bookmarks WHERE season='' AND episode='' AND ((tmdb=? AND tmdb!='') OR (imdb=? AND imdb!=''))''', (str(tmdb or ''), imdb or '')).fetchone()
					if ret_type == 'resume_info': progress = (match[1], match[2])
					else: progress = match[12]
				except: pass
			else:
				try:
					match = dbcur.execute('''SELECT * FROM bookmarks WHERE season=? AND episode=? AND ((tmdb=? AND tmdb!='') OR (imdb=? AND imdb!='') OR (tvdb=? AND tvdb!=''))''', (str(season), str(episode), str(tmdb or ''), imdb or '', str(tvdb or ''))).fetchone()
...
def delete_bookmark(imdb, tvdb='', tmdb='', season='', episode=''):
	try:
		...
		dbcur = get_connection_cursor(dbcon)
		_ensure_bookmarks_table(dbcur)
		dbcur.execute('''DELETE FROM bookmarks WHERE season=? AND episode=? AND ((tmdb=? AND tmdb!='') OR (imdb=? AND imdb!='') OR (tvdb=? AND tvdb!=''))''', ('' if season is None else str(season), '' if episode is None else str(episode), str(tmdb or ''), imdb or '', str(tvdb or '')))
		timestamp = datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%S.000Z")
		dbcur.execute('''INSERT OR REPLACE INTO service Values (?, ?)''', ('last_paused_at', timestamp))
		dbcur.connection.commit()
	except:
...
def clear_bookmarks():
	# Dedicated to just the bookmarks table — delete_punchplay_tables() also resets
